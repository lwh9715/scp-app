	...
	<view class="flex flex-column" v-watermark="watermarkConfig">
		<block v-if="pricelist.length > 0 && isread">
			...
			<uni-table ref="table" stripe emptyText="暂无更多数据">
				<uni-tr>
					<uni-th width="100" align="center">起运港</uni-th>
					<uni-th width="5" align="center">船公司</uni-th>
				...
				<uni-tr v-for="(item,index) in pricelist" :key="index" v-if="pricelist.length > 0 && isread">
					<uni-td align="center">
						<text @click="checkDetail(item)" style="text-decoration: underline;">
							{{ item.pol }}<br>
							<text v-if="item.poa_show != ''">
								驳船: {{ item.poa_show }}</text>
						</text>
					</uni-td>
					<uni-td align="center">
					...
					<uni-td style="color: #ce3c3c;" align="center">
						<text>{{ item.cost20 }}<br>
							<text style="font-size: 12px;" v-if="item.barcost20 != null">
								+ {{ item.barcost20 }}</text>
						</text>
					</uni-td>
					<uni-td style="color: #ce3c3c;" align="center">
						<text>{{ item.cost40gp }}<br>
							<text style="font-size: 12px;" v-if="item.barcost40gp != null">
								+ {{ item.barcost40gp }}</text>
						</text>
					</uni-td>
					<uni-td style="color: #ce3c3c;" align="center">
						<text>{{ item.cost40hq }}<br>
							<text style="font-size: 12px;" v-if="item.barcost40hq != null">
								+ {{ item.barcost40hq }}</text>
						</text>
					</uni-td>
	...
	export default {
		...
		created() {
			...
			if (islogin && user) {
				...
				uni.request({
					url: "http://39.108.229.3:8080/port/log",
					data: {
						pol: this.datatemp.pol,
						pod: this.datatemp.pod,
						inputer: user.data.data.name,
						inputerDept: ""
					},
					method: "GET",
					success: (res) => {},
				...
				this.$H.get('/scp/edi/api?method=fcllist&pol=' + this.datatemp.pol + '&pod=' +
