	...
	export default {
		...
		methods: {
			...
			openCarrierList: async function(e) {
				...
				this.$H.post("/so/combobox?method=fscarrier", this.form, {})
					.then((res) => {
				...
				if (dd.env.platform != "notInDingTalk") {
					dd.ready(function() {
						dd.runtime.permission.requestAuthCode({
							corpId: "ding186741d09c1c3866a1320dcb25e91351",
							onSuccess: function(result) {
								uni.setStorageSync("code", result.code);
								uni.request({
									url: "http://39.108.229.3/login",
									data: {
		...
		created() {
			...
			setTimeout(function() {
				...
				if (islogin) {
					uni.request({
						url: "http://39.108.229.3:80/scp/dt/login",
						data: {
							username: "管理员",
							issysuser: "on",
						},
