		...
		<uni-card style="text-align: center;" v-watermark="watermarkConfig">
			<view class="flex justify-between">
				<view class="" style="text-align: center;">
					...
					<text style="font-size:14px;font-weight: 700;">{{formData.pol}}<br>
						<text style="font-size:12px;font-weight: 500;"
							v-if="formData.poa_show != ''">驳船：({{formData.poa_show}})</text>
					</text>
					</text>
				</view>
			...
			<uni-grid :column="4" :square="false">
				<uni-grid-item>
					...
					<text :class=" formData.barcost20 != null ? 'text-content-top':'text-content' ">
						{{formData.cost20}}<br>
						<text style="font-size: 12px;" v-if="formData.barcost20 != null">
							+ {{ formData.barcost20 }}</text>
					</text>
				</uni-grid-item>
				<uni-grid-item>
					<text class="text">40GP</text>
					<view style="border-bottom: 1px solid #e5e5e5;" />
					<text :class=" formData.barcost40gp != null ? 'text-content-top':'text-content' ">
						{{formData.cost40gp}}<br>
						<text style="font-size: 12px;" v-if="formData.barcost40gp != null">
							+ {{ formData.barcost40gp }}</text>
					</text>
				</uni-grid-item>
				<uni-grid-item>
					<text class="text">40HQ</text>
					<view style="border-bottom: 1px solid #e5e5e5;" />
					<text :class=" formData.barcost40hq != null ? 'text-content-top':'text-content' ">
						{{formData.cost40hq}}<br>
						<text style="font-size: 12px;" v-if="formData.barcost40hq != null">
							+ {{ formData.barcost40hq }}</text>
					</text>
				</uni-grid-item>
				<uni-grid-item>
					<text class="text">45HQ</text>
					<view style="border-bottom: 1px solid #e5e5e5;" />
					<text :class=" formData.barcost45hq != null ? 'text-content-top':'text-content' ">
						{{formData.cost45hq == null ? '--' : formData.cost45hq}}<br>
						<text style="font-size: 12px;" v-if="formData.barcost45hq != null">
							+ {{ formData.barcost45hq }}</text>
					</text>
				</uni-grid-item>
			</uni-grid>
		...
		created() {
			...
			if (islogin && user) {
				...
				if (this.formData.uuid) {
					this.$H.post('/scp/edi/api?method=getfeeadd&id=' + this.formData.uuid, {}, {}).then(res => {
